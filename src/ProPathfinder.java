// ProPathfinder - An Interactive Visualisation Tool for Pathfinding Algorithms
...
public class ProPathfinder
{
    ...
    //LABELS
    JLabel algL = new JLabel("Pathfinding Algorithm:");
    ...
    //BUTTONS
    JButton searchB = new JButton("BEGIN SEARCH");
    ...
    {    //INITIALISE THE GUI ELEMENTS
        ...
        toolP.setBorder(BorderFactory.createCompoundBorder(outer, inner));
        toolP.setBackground(new Color(244, 185, 113));
        int space = 25;
        int buff = 45;
        ...
        searchB.setBounds(40, space, 140, 25);
        searchB.setFont(new Font(Font.DIALOG, Font.BOLD | Font.ITALIC,  12));
        searchB.setBackground(new Color(128, 13, 0));
        searchB.setForeground(Color.white);
        toolP.add(searchB);
        space += buff;

        resetB.setBounds(40, space, 140, 25);
        resetB.setFont(new Font(Font.DIALOG, Font.BOLD,  12));
        resetB.setBackground(Color.black);
        resetB.setForeground(Color.white);
        toolP.add(resetB);
        space += buff;

        genMapB.setBounds(40, space, 140, 25);
        genMapB.setFont(new Font(Font.DIALOG, Font.BOLD,  12));
        genMapB.setBackground(Color.black);
        genMapB.setForeground(Color.white);
        toolP.add(genMapB);
        space += buff;

        clearMapB.setBounds(40, space, 140, 25);
        clearMapB.setFont(new Font(Font.DIALOG, Font.BOLD,  12));
        clearMapB.setBackground(Color.black);
        clearMapB.setForeground(Color.white);
        toolP.add(clearMapB);
        space += 40;

        algL.setBounds(36, space, 170, 25);
        toolP.add(algL);
        space += 25;

        algorithmsBx.setBounds(40, space, 120, 25);
        algorithmsBx.setFont(new Font(Font.SANS_SERIF, Font.BOLD | Font.ITALIC,  12));
        toolP.add(algorithmsBx);
        space += 30;

        toolL.setHorizontalAlignment(JLabel.CENTER);
        toolL.setBounds(40, space, 120, 25);
        toolP.add(toolL);
        space += 25;

        toolBx.setBounds(40, space, 120, 25);
        toolBx.setFont(new Font(Font.SANS_SERIF, Font.BOLD | Font.ITALIC,  12));
        toolP.add(toolBx);
        space += buff;

        sizeL.setBounds(12, space, 40, 25);
        toolP.add(sizeL);
        size.setMajorTickSpacing(10);
        size.setBounds(62, space, 100, 25);
        toolP.add(size);
        cellsL.setBounds(164, space, 40, 25);
        toolP.add(cellsL);
        space += buff;

        delayL.setBounds(12, space, 50, 25);
        toolP.add(delayL);
        speed.setMajorTickSpacing(5);
        speed.setBounds(62, space, 100, 25);
        toolP.add(speed);
        msL.setBounds(164, space, 40, 25);
        toolP.add(msL);
        space += buff;

        obstacleL.setBounds(12, space, 100, 25);
        toolP.add(obstacleL);
        obstacles.setMajorTickSpacing(5);
        obstacles.setBounds(62, space, 100, 25);
        toolP.add(obstacles);
        densityL.setBounds(164, space, 100, 25);
        toolP.add(densityL);
        space += buff;
        ...
        timeL.setBounds(15, space, 140, 25);
        toolP.add(timeL);
        space += buff;

        aboutB.setBounds(40, space, 120, 25);
        toolP.add(aboutB);
        ...
        aboutB.addActionListener(new ActionListener()
        {
            ...
            public void actionPerformed(ActionEvent e)
            {
                JOptionPane.showMessageDialog(frame, "	                           ProPathfinder\n"
                        + "- An Interactive Path-finding Visualisation Tool\n"
    ...
    {    //ALGORITHM CLASS
        ...
        //IT USES A PRIORITY QUEUE TO KEEP TRACK OF NODES THAT IT NEEDS TO EXPLORE
        //EACH NODE IN THE PRIORITY QUEUE IS EXPLORED AND ALL OF ITS NEIGHBORS ARE ADDED TO THE QUEUE
        //ONCE A NODE IS EXPLORED IT IS DELETED FROM THE QUEUE
        //AN ARRAYLIST IS USED TO REPRESENT THE PRIORITY QUEUE
        //A SEPARATE ARRAYLIST IS RETURNED FROM A METHOD THAT EXPLORES A NODES NEIGHBORS
        //THIS ARRAYLIST CONTAINS ALL THE NODES THAT WERE EXPLORED, IT IS THEN ADDED TO THE QUEUE
        //A HOPS VARIABLE IN EACH NODE REPRESENTS THE NUMBER OF NODES TRAVELED FROM THE START
        public void Dijkstra()
        {
            long startTime = System.currentTimeMillis();

            ArrayList<Node> priority = new ArrayList<Node>();    //CREATE A PRIORITY QUEUE
            priority.add(map[startx][starty]);    //ADD THE START TO THE QUEUE
            while(solving)
            {
                time = (System.currentTimeMillis() - startTime);

                if(priority.size() <= 0)
                {    //IF THE QUEUE IS 0 THEN NO PATH CAN BE FOUND
                    solving = false;
                    break;
                }
                ...
                if(explored.size() > 0)
                {
                    ...
                    priority.addAll(explored);    //ADD ALL THE NEW NODES TO THE QUEUE
                    Update();
                    delay();
                } else
                {    //IF NO NODES WERE EXPLORED THEN JUST REMOVE THE NODE FROM THE QUEUE
                    priority.remove(0);
                }
            }
        ...
        //A STAR WORKS ESSENTIALLY THE SAME AS DIJKSTRA CREATING A PRIORITY QUEUE AND PROPAGATING OUTWARDS UNTIL IT FINDS THE END
        ...
        //THIS HEURISTIC IS BUILT IN BY SORTING THE QUEUE ACCORDING TO HOPS PLUS DISTANCE UNTIL THE FINISH
        public void AStar()
        {
            long startTime = System.currentTimeMillis();
            ...
            while(solving)
            {
                ...
                sortQueue(priority);    //SORT THE PRIORITY QUEUE
            }
